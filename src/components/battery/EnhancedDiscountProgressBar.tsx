...
const EnhancedDiscountProgressBar: React.FC<EnhancedDiscountProgressBarProps> = ({ 
  ...
  for (let i = discountTiers.length - 1; i >= 0; i--) {
    ...
    if (tier && currentTotal >= tier.threshold) {
      currentTierIndex = i;
      nextTierIndex = i + 1;
      break;
    }
  ...
  if (currentTierIndex === -1 && currentTotal > 0 && discountTiers[0]) {
    ...
    // Between tiers
    const currentTierThreshold = discountTiers[currentTierIndex]?.threshold || 0;
    const nextTierThreshold = discountTiers[nextTierIndex]?.threshold || 0;
  ...
  React.useEffect(() => {
    ...
    if (currentTotal > previousTotal.current) {
      ...
      for (let i = 0; i < discountTiers.length; i++) {
        ...
        if (threshold && currentTotal >= threshold && previousTotal.current < threshold) {
          // We crossed this threshold
          setShowConfetti(true);
          fireConfetti(discountTiers[i]?.discount || '0%');
          break;
        }
      }
    }
