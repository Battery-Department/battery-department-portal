...
const EnhancedRuntimeCalculator2: React.FC<RuntimeCalculatorProps> = ({
  ...
  const configurations = [
    { 
      ...
      runtime: calculateRuntime(runtimeData?.batteries?.[selectedBattery] || 0, selectedToolObj?.consumption || 1).toFixed(1) + ' hrs',
    ...
    { 
      ...
      label: 'Economy', 
      batteries: '2× 6Ah', 
      runtime: (2 * calculateRuntime(runtimeData?.batteries?.['6Ah'] || 0, selectedToolObj?.consumption || 1)).toFixed(1) + ' hrs',
      cost: '$190',
      efficiency: 85,
      weight: 'Medium' 
    },
    { 
      ...
      label: 'Max Runtime', 
      batteries: '1× 15Ah', 
      runtime: calculateRuntime(runtimeData?.batteries?.['15Ah'] || 0, selectedToolObj?.consumption || 1).toFixed(1) + ' hrs',
      cost: '$196',
      efficiency: 88,
      weight: 'Heavy' 
    }
  ...
  const renderBatteryVisual = (configId: string) => {
    ...
    const batteryCount = parseInt(config?.batteries?.split('×')?.[0]?.trim() || '1');
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      {selectedTool && (
                        <div className="mt-1 text-xs text-blue-800 font-medium">
                          {calculateRuntime(
                            runtimeData?.batteries?.[battery as keyof typeof runtimeData.batteries] || 0,
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  ...
                  <span className="font-medium text-blue-700">
                    {calculateRuntime(
                      runtimeData?.batteries?.[selectedBattery as keyof typeof runtimeData.batteries] || 0,
                      selectedToolObj?.consumption || 1
                    ).toFixed(1)} hours
                  </span>.
                </p>
                ...
                {getRecommendation(selectedTool as any, selectedBattery) && (
                  <div className="mt-2 text-xs text-blue-700">
                    <span className="font-medium">Recommendation:</span> {getRecommendation(selectedTool as any, selectedBattery)}
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <div className="text-xs text-gray-600 mt-1">
                Powers up to {((runtimeData?.batteries?.[selectedBattery as keyof typeof runtimeData.batteries] || 0) / 2).toFixed(1)} hours of average use
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="config-details">
              ...
              <div className="grid grid-cols-2 gap-y-2 gap-x-4 mt-3 text-sm">
                ...
                <div className="flex items-center">
                  ...
                  <span>Efficiency: <strong>{getConfigById(selectedConfig)?.efficiency || 0}%</strong></span>
                </div>
